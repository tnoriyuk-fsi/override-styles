...
自前コードの静的セキュリティ解析（SAST）として **CodeQL** を導入している（[`codeql.yml`](https://github.com/tnoriyuk-fsi/override-styles/blob/main/.github/workflows/codeql.yml)）。`audit-ci`（依存パッケージの脆弱性）とは別レイヤで、**自分たちが書いた TypeScript/JavaScript コードの脆弱性**を解析する。

- 言語は `javascript-typescript`。`push`（main）/ `pull_request` / 週次スケジュールで実行する。
- 結果は GitHub の **Security タブ（code scanning alerts）** に集約され、PR の差分上にも表示される。
- クエリは既定スイート（高精度・低ノイズ）で運用する。検出を広げたい場合は `init` の `queries: security-extended` を指定する。
- アラートは **修正する**か、誤検知・該当しないものは**理由を付けて dismiss する**（`audit-ci` の allowlist 運用に準じる）。
- code scanning のブランチ保護必須化は現状行わず、可視化（アラート集約）に留める。
